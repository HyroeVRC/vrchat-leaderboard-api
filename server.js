...
const cleanName = (s) => {
  ...
  // On garde tel quel pour afficher proprement; la clef en DB est le nom exact.
  return s;
};
...
app.get("/commit", async (req, res) => {
  ...
  let name = cleanName(data.slice(0, idx));
  let msStr = data.slice(idx + 1).replace(/[^0-9]/g, "");
  if (!msStr) msStr = "0";

  let total_ms = Number(msStr);
  if (!Number.isFinite(total_ms) || total_ms < 0) total_ms = 0;
  if (total_ms > 1e13) total_ms = 1e13;
...
app.get("/p/:k", (req, res) => {
  ...
  const s = ensureSess(ip);
  if (s.packBuf.length < 64 + 1 + 16) s.packBuf += ALPHABET[k]; // 64 nom + '#' + 16 ms
  res.send("ok\n");
});
